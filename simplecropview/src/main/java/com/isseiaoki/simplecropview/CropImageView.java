...
@SuppressWarnings("unused") public class CropImageView extends ImageView {
  ...
  private Bitmap mCustomIcon = null;
  private int mCustomIconTintColor = TRANSLUCENT_WHITE;
  private int mOverlayAreaColor = TRANSLUCENT_WHITE;
  private boolean mIsOnlyRB = false;
  private int mSizeIcon = 0;
  private int mSizeIconMargin = 0;
  private boolean mIsGuidelines = false;
  private boolean mIsGuideBorderlineTouchDown = false;
  ...
  private boolean mIsDebugAdvance = false;
  ...
  private MoveUpCallback mMoveUpCallback = null;
  ...
  @Override public Parcelable onSaveInstanceState() {
    ...
    ss.customIcon = this.mCustomIcon;
    ss.isOnlyRB = this.mIsOnlyRB;
    ss.sizeIcon = this.mSizeIcon;
    ss.isGuidelines = this.mIsGuidelines;
    ss.isGuideBorderlineTouchDown = this.mIsGuideBorderlineTouchDown;
    return ss;
  }
  ...
  @Override public void onRestoreInstanceState(Parcelable state) {
    ...
    this.mIsDebugAdvance = ss.isDebugAdvance;
    ...
    this.mCustomIcon  = ss.customIcon;
    this.mIsOnlyRB = ss.isOnlyRB;
    this.mSizeIcon = ss.sizeIcon;
    this.mIsGuidelines = ss.isGuidelines;
    this.mIsGuideBorderlineTouchDown = ss.isGuideBorderlineTouchDown;
  }
  ...
  private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
    ...
    try {
      ...
      mIsGuidelines = ta.getBoolean(R.styleable.scv_CropImageView_scv_guidelines_enabled, true);
      mIsGuideBorderlineTouchDown = ta.getBoolean(R.styleable.scv_CropImageView_scv_guide_borderline_touch_down_enabled, true);
      mSizeIcon = ta.getDimensionPixelSize(R.styleable.scv_CropImageView_scv_size_icon, (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
      mSizeIconMargin = ta.getDimensionPixelSize(R.styleable.scv_CropImageView_scv_size_icon_margin, (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
  ...
  private void drawDebugAdvanceInfo(Canvas canvas) {
    Paint.FontMetrics fontMetrics = mPaintDebug.getFontMetrics();
    mPaintDebug.measureText("W");
    int textHeight = (int) (fontMetrics.descent - fontMetrics.ascent);
    int x = (int) (mImageRect.left + (float) mHandleSize * 0.5f * getDensity());
...
//      builder = new StringBuilder();
//      builder.append(calculateQRCode());
//      y += textHeight;
//      for (String line: builder.toString().split("\n")) {
//        mPaintDebug.setColor(WHITE);
//        canvas.drawText(line, x, y, mPaintDebug);
//        y += -mPaintDebug.ascent() + mPaintDebug.descent();
//      }
    }
  }
  ...
  private void drawHandles(Canvas canvas) {
    ...
    if (mCustomIcon==null){
      ...
      Paint m = new Paint();
      m.setColorFilter(new PorterDuffColorFilter(mCustomIconTintColor, PorterDuff.Mode.SRC_IN));
      Rect rectangle = new Rect(Math.round(mFrameRect.right) - (mSizeIcon+mSizeIconMargin),Math.round(mFrameRect.bottom) - (mSizeIcon+mSizeIconMargin)  , Math.round(mFrameRect.right) - mSizeIconMargin, Math.round(mFrameRect.bottom)-mSizeIconMargin);
      canvas.drawBitmap(mCustomIcon,null,rectangle,m);
    }
  }
  ...
  private boolean isInsideCornerRightBottom(float x, float y) {
    float dx = x - (mFrameRect.right - (mIsOnlyRB ? mSizeIcon : 0));
    float dy = y - (mFrameRect.bottom - (mIsOnlyRB ? mSizeIcon : 0));
    float d = dx * dx + dy * dy;
    return sq(mHandleSize + mTouchPadding) >= d;
  }
    ...
    if (mCropMode == CropMode.FREE) {
      ...
      if (mIsOnlyRB){
        if (diffX>=0){
          ...
          mFrameRect.left += Math.abs(diffX);
        }
        if (diffY>=0){
          mFrameRect.top += -diffY;
        }else{
          mFrameRect.top += Math.abs(diffY);
        }
      }
      mFrameRect.bottom += diffY;
      Log.d(TAG,mFrameRect.toString());
      Log.d(TAG,diffX+" - "+diffY);
      Log.d(TAG,Math.abs(diffX)+" - "+Math.abs(diffY));
      if (isWidthTooSmall()) {
        float offsetX = mMinFrameSize - getFrameW();
        mFrameRect.right += offsetX;
        if (mIsOnlyRB){
  ...
  // Calculate medium large complex
  private Map<Integer,String> calculateQRCode(){
    final Map<Integer,String> mMap = new HashMap<>();
    Log.d(TAG,"workload " + getFrameW() + "-"+ getFrameH());
    float mWorkload = (getFrameW() + getFrameH())/600;
    Log.d(TAG,"workload "+mWorkload);
    int mResult = Math.round(mWorkload*100);
    Log.d(TAG,"workload "+mResult);
    //too long
    boolean mTinyWidth = getFrameW() <= 130;
    boolean mTinyWidthAndResult = getFrameW() <= mResult;
    boolean mTinyHeight = getFrameH() <= 130;
    boolean mTinyHeightAndResult = getFrameH() <= mResult;
    String mValue = "";
    ...
    mMap.put(1,mValue);
    return mMap;
    //return String.format(mScannerWorkLoad,mResult+"%",mValue);
  }
  ...
  private void resetImageInfo() {
    ...
    mSizeIconMargin = 0;
    mIsGuidelines = false;
    mIsGuideBorderlineTouchDown = false;
    mInputImageWidth = 0;
    mInputImageHeight = 0;
    mOutputImageWidth = 0;
  ...
  public void pointMoveUp(final MoveUpCallback moveUpCallback){
  ...
  private void onRelease(){
    if (mMoveUpCallback != null) mMoveUpCallback.onRelease();
  }

  private void onDown(){
    if (mMoveUpCallback != null) mMoveUpCallback.onDown();
  }

  public MoveUpRequest moveUp() {
    return new MoveUpRequest(this);
  }
...
//    float b = Math.min(mImageRect.bottom, frameRect.bottom);
    frameRect.set(initialFrameRect.left, initialFrameRect.top, initialFrameRect.right, initialFrameRect.bottom);
    return frameRect;
  }
   ...
   * @param debug is logging enabled
   */
  public void setDebugAdvance(boolean debug) {
    mIsDebugAdvance = debug;
    Logger.enabled = true;
    invalidate();
  }
  ...
  public static class SavedState extends BaseSavedState {
    ...
    boolean isOnlyRB;
    int sizeIcon;
    boolean isGuidelines;
    boolean isGuideBorderlineTouchDown;
    Bitmap.CompressFormat compressFormat;
    int compressQuality;
    boolean isDebug;
    boolean isDebugAdvance;
    int outputMaxWidth;
    int outputMaxHeight;
    int outputWidth;
    int outputHeight;
    ...
    private SavedState(Parcel in) {
      ...
      customIcon = Bitmap.CREATOR.createFromParcel(in);
      isOnlyRB = (in.readInt() != 0);
      sizeIcon = in.readInt();
      isGuidelines = (in.readInt() != 0);
      isGuideBorderlineTouchDown = (in.readInt() != 0);
    }
    ...
    @Override public void writeToParcel(Parcel out, int flag) {
      ...
      out.writeInt(isDebugAdvance ? 1 : 0);
      ...
      out.writeInt(isGuideBorderlineTouchDown ? 1:0);
    }
