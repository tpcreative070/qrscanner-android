...
class Theme : Serializable {
    ...
    fun getDefaultThemeList(): MutableList<Theme?>? {
        try {
            val result: String = PrefsController.getString(QRScannerApplication.Companion.getInstance().getString(R.string.key_theme_list), null)
    ...
    fun getList(): MutableList<Theme?>? {
        try {
            ...
            val current_code_version: Int = PrefsController.getInt(QRScannerApplication.Companion.getInstance().getString(R.string.key_current_code_version), 0)
            return if (value != null && current_code_version == BuildConfig.VERSION_CODE) {
                ...
                PrefsController.putString(QRScannerApplication.Companion.getInstance().getString(R.string.key_theme_list), Gson().toJson(mList))
                PrefsController.putInt(QRScannerApplication.Companion.getInstance().getString(R.string.key_current_code_version), BuildConfig.VERSION_CODE)
                PrefsController.putBoolean(QRScannerApplication.Companion.getInstance().getString(R.string.we_are_a_team), false)
